...
import { Activity, Map as MapIcon, PlusSquare, List, Database, Menu, Wifi, WifiOff } from "lucide-react";
    ...
    <div className="flex flex-col h-full bg-sidebar border-r border-sidebar-border text-sidebar-foreground w-64 p-4">
      ...
      <nav className="flex-1 space-y-1">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors ${active ? "bg-sidebar-accent text-sidebar-accent-foreground font-medium" : "text-muted-foreground hover:bg-sidebar-accent/50 hover:text-foreground"}`}>
      ...
      <div className="mt-auto border-t border-sidebar-border pt-4">
        <div className="flex flex-col gap-2">
          <div className={`flex items-center gap-2 text-xs font-mono px-3 py-2 rounded-md ${isOnline ? "bg-green-500/10 text-green-500" : "bg-destructive/10 text-destructive"}`}>
