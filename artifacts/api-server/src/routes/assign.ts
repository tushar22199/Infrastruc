...
router.post("/inspections/:id/assign", async (req, res) => {
  ...
  const { userId: assigneeId, displayName: assigneeName } = req.body as { userId?: unknown; displayName?: unknown };
  if (typeof assigneeId !== "string" || typeof assigneeName !== "string") {
    res.status(400).json({ error: "userId and displayName are required strings" });
    return;
  }
  ...
  try {
    ...
    const assignerName = req.isAuthenticated()
      ? `${req.user.firstName ?? ""} ${req.user.lastName ?? ""}`.trim() || "A project lead"
      : "A project lead";
    const assignerId = req.isAuthenticated() ? req.user.id : null;
    ...
    notificationsTable &&
      db.insert(notificationsTable)
        ...
        .execute()
        .catch((err: unknown) => req.log.error({ err }, "Failed to create assignment notification"));
    ...
    db.insert(activityLogTable)
      ...
      .execute()
      .catch((err: unknown) => req.log.error({ err }, "Failed to log assignment activity"));
